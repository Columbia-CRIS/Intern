...
"""
"""
This file simulates wealth with different tax
"""
...
def level_to_salary(x):
    return np.asarray(utility.danmark_income).astype(float)[x]
...
if __name__ == '__main__':
    ...
    while loss > epsilon and epoch < epoch_max:
        ...
        # if (epoch == 4):
            # print(utility.best_fit_distribution(agent_income_list))
        # if (epoch < 5 or epoch %5 == 0):
            # plot()
            # utility.plot_wealth_save(agent_wealth_list, "Wealth -overlay -no tax -100k -Start with Danish Income" + str(epoch))
            # utility.plot_save(num_levels, num_classes, count_levels_list, count_levels_combined,"Income -overlay -no tax -100k -Start with Danish Income" + str(epoch))


    utility.plot_wealth(agent_wealth_list, "Wealth -overlay -no tax -100k -Start with Danish Income" + str(epoch))
