...
"""
"""
This files checks the welath distribution when there is a constant growth rate
"""
...
agent_wealth_list: ndarray = np.zeros(num_agents) # Total income cumulated over time
...
def turtle():
    ...
    for i in range(agent_wealth_list.size):
        agent_wealth_list[i] = agent_wealth_list[i] * 1.2
...
if __name__ == '__main__':
    ...
    while loss > epsilon and epoch < epoch_max:
        ...
        epoch += 1
    utility.plot(num_levels,num_classes, count_levels_list,count_levels_combined)
    utility.plot_wealth(agent_wealth_list, "Wealth Histogram with Constant Growth One Class")
